...
| srv_flight | the fifteen `srv_flight.h` handlers stay safe and free of UB over an unconstrained ClientHello, selection and session — every pointer NULL or live, every length any `size_t` — and every one of them that returns anything but `CH_OK` leaves `h->alert` holding a description somebody wrote. It also proves that a checked retry hello asks for no second `HelloRetryRequest`, that the key exchange wipes the scalar and the public value on both exits, and that `srv_complete` reaches `CH_ST_CONNECTED`. This is `srv_accept`'s layering turned around: here the handlers are real and the parser, the nine builders, the cookie, the two authentication entry points, the record reader, the key schedule, the record layer and the I/O shim are contract stubs, so **no message content is proved here either** — what the builders write is the `srv_message` leg's, and the cookie is `srv_cookie`'s; `ROLE=server` only | chains ≤ 2 certificates of ≤ 8 B, messages ≤ 32 B, peer record limit ≤ 16 B |
| srv_parser | the ClientHello parser (`srv_parse_client_hello` with `srv_ext_known` and `srv_ext_duplicate`) stays safe on hostile bytes with no ALPN offer, and an accepted hello honours `srv_parser.h`'s contract: the caller's seeded alert stands, `supported_versions` was read, every share, cookie and server_name pointer lies inside the message with its length, a share is `CH_KEX_CLIENT_SHARE` bytes and `shares` reports it, `truncated_len` lies inside the message, the seen mask holds §9.2's pairings, a `record_size_limit` is 0 or at least 64, and every share's group was listed in `supported_groups`. A refusal writes one of the four descriptions the header lists and no other. SHA-256 is a contract stub, so **what the frozen digest covers is tested, not proved** (`test/srv_parser_tests.h`); `ROLE=server` only | messages ≤ 256 B, no ALPN offer |
