...
| peak transient stack, handshake (one x25519 ladder) | ~2.6 k |
| peak transient stack, steady state | < 1 k |
...
| hkdf | extract/expand/expand-label no UB, incl. hash-the-key path | keys ≤ 131 B, out ≤ 64 B |
...
| aead | seal/open round-trip, forged tag ⇒ zero bytes written | pt ≤ 64 B, aad ≤ 32 B |
| x25519 | field ops no UB/overflow at generous limb bounds (2^24) | per-op |
| record | rec_open safe on fully hostile bytes; round-trip preserved | records ≤ 96 B |
