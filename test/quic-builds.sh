...
# so this is the path.
#
# bin/quic_stub_test compiles all eight QUIC_SRCS and the test main that
# calls every stub, so it is the target that fails when a QUIC source
# stops compiling. INV-26 turns a whole class of edit into that failure:
# ch_quic stores no aes_public_key, quic_aes.h leaves the type
...
cd "$(dirname "$0")/.." || exit 1
exec make -s bin/quic_stub_test
