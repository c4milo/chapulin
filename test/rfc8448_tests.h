...
static void rfc8448_check_cv(const uint8_t th_cert[SHA256_LEN]) {
    ...
    sha256_update(&h, th_cert, SHA256_LEN);
    sha256_final(&h, mh);

    // The 2048-bit floor must refuse the RFC's RSA-1024 key outright.
    CHECK(rsa_pss_verify(rfc8448_rsa_n, sizeof rfc8448_rsa_n, mh, sig, 128) == 0);
...
static void test_rfc8448_1rtt(void) {
    ...
    uint8_t bk[SHA256_LEN];
    ks_early(zeros, sizeof zeros, 0, early, bk);
    CHECK(memcmp(early, rfc8448_s3_early_secret, SHA256_LEN) == 0);

    // CH..SH snapshot feeds the handshake traffic secrets.
    sha256 th;
    sha256_init(&th);
    sha256_update(&th, rfc8448_s3_client_hello, sizeof rfc8448_s3_client_hello);
    sha256_update(&th, rfc8448_s3_server_hello, sizeof rfc8448_s3_server_hello);
    uint8_t h[SHA256_LEN];
    rfc8448_snapshot(&th, h);
    CHECK(memcmp(h, rfc8448_s3_th_ch_sh, SHA256_LEN) == 0);
    uint8_t hs[SHA256_LEN];
    uint8_t c_hs[SHA256_LEN];
    uint8_t s_hs[SHA256_LEN];
    ks_handshake(early, shared, h, hs, c_hs, s_hs);
    CHECK(memcmp(hs, rfc8448_s3_hs_secret, SHA256_LEN) == 0);
    ...
    sha256_update(&th, rfc8448_s3_ee, sizeof rfc8448_s3_ee);
    sha256_update(&th, rfc8448_s3_cert, sizeof rfc8448_s3_cert);
    rfc8448_snapshot(&th, h);
    rfc8448_check_cv(h);

    // CH..CertificateVerify snapshot is what the server Finished MACs.
    sha256_update(&th, rfc8448_s3_cv, sizeof rfc8448_s3_cv);
    rfc8448_snapshot(&th, h);
    ...
    sha256_update(&th, rfc8448_s3_client_fin_msg, sizeof rfc8448_s3_client_fin_msg);
    rfc8448_snapshot(&th, h);
...
static void test_rfc8448_binder(void) {
    uint8_t early[SHA256_LEN];
    uint8_t bk[SHA256_LEN];
    ks_early(rfc8448_s3_res_psk, sizeof rfc8448_s3_res_psk, 1, early, bk);
    CHECK(memcmp(early, rfc8448_s4_early_secret, SHA256_LEN) == 0);
    CHECK(memcmp(bk, rfc8448_s4_binder_key, SHA256_LEN) == 0);
...
static void test_rfc8448_hrr(void) {
    ...
    sha256 th;
    sha256_init(&th);
    sha256_update(&th, synth_hdr, sizeof synth_hdr);
    sha256_update(&th, h1, SHA256_LEN);
    sha256_update(&th, rfc8448_s5_hrr, sizeof rfc8448_s5_hrr);
    sha256_update(&th, rfc8448_s5_ch2, sizeof rfc8448_s5_ch2);
    sha256_update(&th, rfc8448_s5_sh, sizeof rfc8448_s5_sh);
    uint8_t h[SHA256_LEN];
    sha256_final(&th, h);
    ...
    uint8_t c_hs[SHA256_LEN];
    uint8_t s_hs[SHA256_LEN];
    ks_handshake(early, rfc8448_s5_ecdhe, h, hs, c_hs, s_hs);
