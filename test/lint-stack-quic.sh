#!/usr/bin/env bash
# The catch target for the TRANSPORT=quic frame-budget violation in
...
# the lint objecting.
#
# The TRANSPORT=quic object packages the eight QUIC_SRCS, which every
...
cd "$(dirname "$0")/.." || exit 1
exec make -s lint-stack TRANSPORT=quic
