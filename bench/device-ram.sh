...
# deriving them from a hosted build: cross-compiles the sixteen library
# sources (no tests) at -Os, reads flash from each object's .text+.rodata
# sections, and takes worst-case frames from -fstack-usage. The same
...
# The sixteen sources the Makefile links into bin/chapulin.o.
SRCS="ct sha256 hkdf chacha20 poly1305 aead x25519 p256 buf record keysched \
