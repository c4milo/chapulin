...
inductive Verdict where
  /-- The chain verified: the leaf's key algorithm and key. -/
...
inductive Step where
  /-- An anchor named the issuer and verified the signature. -/
  | reached
  /-- `CH_EPROTO`. -/
  | rejected
...
/-- Step 6a: some anchor's subject Name equals `cert`'s issuer Name and
its key verifies `cert`'s signature. An anchor whose SubjectPublicKeyInfo
the reader refuses verifies nothing. -/
def anchorVerifies (cfg : Config) (cert : ByteArray) (c : Certificate) : Bool :=
  cfg.anchors.any fun a =>
...
/-- The corpus r2 chain accepted, and one refusal per rule: the clock one
...
def selftest : Bool :=
  ...
  (match ok with
   | .ok alg key => alg == .p256 && key == leafKey && key.size == 64
   | _ => false) &&
    ok.name == "ok" &&
...
theorem walkFrom_reached (cfg : Config) (rest : List ByteArray) :
  ...
  | cons next more ih =>
    ...
    by_cases h_anchor : anchorVerifies cfg cert c
    · exact .anchor h_anchor
    · simp only [h_anchor, Bool.false_eq_true, if_false] at h
      split at h
      · simp at h
      rename_i h_read
      split at h
...
a verified signature path to an anchor; and the key the verdict carries
is that leaf's own. -/
theorem verifyChain_ok (cfg : Config) (list : ByteArray) (alg : KeyAlg) (key : ByteArray)
    (h : verifyChain cfg list = .ok alg key) :
    ∃ leafBytes rest leaf,
